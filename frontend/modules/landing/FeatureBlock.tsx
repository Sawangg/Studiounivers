...
export const FeatureBlock: React.FC<FeatureBlockProps> = ({ title, description, imagePath, ...props }) => (
    <div className="w-full block max-w-md 2xl:max-w-[20rem] p-7 3xl:p-10 overflow-hidden md:bg-white-300" {...props}>
        <Image src={imagePath} width="24px" height="24px" alt="icon" />
        <h4 className="font-title text-xl 3xl:text-2xl my-2">{title}</h4>
        <p className="w-full text-md 3xl:text-lg break-words">{description}</p>
    </div>
);
